...
ILTree dummy = { DUMMYNode, 0, 0, NULL, NULL };
...
tree MakeLeaf(int Kind, int N){
	...
	p->NodeKind = Kind;     
	p->IntVal = N;
	p->NodeOpType = 0;				// default to zero/null, because
	p->LeftC = p->RightC = NULL;	// leaving them uninitialized is a bad memory habit.
	return (p);
}
...
tree MakeTree(int NodeOp, tree Left, tree Right){
	...
	p->NodeKind = EXPRNode; 
	p->NodeOpType = NodeOp;
	p->IntVal = 0; // IntVal is used for reference count for internal node.
	p->LeftC = Left;
	p->RightC = Right;
	// for TypeId subtree, we need to increment its reference count
	if (p->LeftC->NodeOpType == TypeIdOp) p->LeftC->IntVal++;	
	if (p->RightC->NodeOpType == TypeIdOp) p->RightC->IntVal++;
	return (p);
}
...
********************************************************/
int IsNull(tree T){
	return (T == NULL || NodeKind(T) == DUMMYNode );
}
...
/****************************************************************
* 	This function clears all the memory of the given tree.	*
*   One may leave the alloc'd memory unfree'd and have no 	*
*   problem, but this practice causes memory leak, and 		*
*	therefore is not a good one.							*
****************************************************************/
void deleteTree(tree* Tptr){
	tree T = *Tptr;
	if (T==NULL || IsNull(T) ) return;
	if (T->NodeOpType==TypeIdOp && T->IntVal>1){
		T->IntVal--;
		return;
	}
	deleteTree(&(T->LeftC));
...
static int crosses[SCREEN_WIDTH];
...
char* getname(int i){/*return ID name or String, i is the index of the string table, passed through yylval*/
	return( str_table+i );/*return string table indexed at i*/
}
...
void printtree (tree nd, int depth){
	...
	if (!depth)
	{
		memset(crosses, 0, SCREEN_WIDTH*sizeof(int));
	...
	switch (NodeKind (nd))
	{
		case IDNode:    
			...
			if (indx >= 0){
				...
				fprintf (treelst,"[IDNode,%d,\"%s\"]\n", indx, "err");
			break;
		// // Only uncomment following lines when in project 3! (i.e., when you included proj3.h)
		// case STNode:
		// 	indx = IntVal(nd);
		// 	if (indx > 0){
...
//   // search the str_table to locate 'str'
//   return is_in_str_table(str);
// }
